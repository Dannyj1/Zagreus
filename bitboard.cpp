...
namespace Zagreus {
    ...
    bool Bitboard::isIsolatedPawn(int square, PieceColor pawnColor) {
        ...
        if (square % 8 != 0) {
            neighborMask |= rayAttacks[Direction::NORTH][square - 1] | rayAttacks[Direction::SOUTH][square - 1] |
                            (1ULL << (square - 1));
        }
...
//            assert(capturedPiece != PieceType::BLACK_KING);
    ...
    int Bitboard::getLeastValuableAttackerWeight(int attackedSquare, PieceColor color) {
        int leastValuableWeight = 200000;
...
//        int mvv = getMostValuableVictimWeight(attackedSquare, attackingColor);
    ...
    int Bitboard::getMostValuableVictimWeight(int attackedSquare, PieceColor attackingColor) {
        ...
        int mostValuableWeight =
                getPieceColor(pieceOnSquare) == getOppositeColor(attackingColor) ? getPieceWeight(pieceOnSquare) : -1;
