...
namespace Chess {
    ...
    int Bishop::getEvaluationScore(Board* board) {
        ...
        for (Tile* tile : attackedTiles) {
            if (Chess::Board::isInCenter(tile->getX(), tile->getY())) {
                score += 2;
            } else if (Chess::Board::isInExtendedCenter(tile->getX(), tile->getY())) {
                score += 1;
            }
        }
        ...
        if (isFianchetto) {
            for (Direction direction : getColor() == PieceColor::WHITE ? WHITE_FIANCHETTO_DIRECTIONS
                ...
                if (tile->getPiece() && tile->getPiece()->getPieceType() == PieceType::PAWN &&
    ...
    void King::getPseudoLegalMoves(std::vector<Tile*> &result, Board* board) {
        ...
        for (Direction direction : ALL_DIRECTIONS) {
            ...
            if (tile != nullptr) {
                if ((tile->getPiece() == nullptr || (tile->getPiece()->getPieceType() != PieceType::KING &&
        ...
        if (!getHasMoved() && !isChecked(board)) {
            ...
            if (aRookTile != nullptr) {
                ...
                if (aRook != nullptr && !aRook->getHasMoved()) {
                    ...
                    for (int i = 1; i < 4; i++) {
                        Tile* tile = board->getTileUnsafe(loc.x - i, loc.y);
                    ...
                    if (canCastle) {
                        result.push_back(board->getTileUnsafe(aRookLoc.x, aRookLoc.y));
                    }
                }
            }

            if (hRookTile != nullptr) {
                Piece* hRook = board->getTileUnsafe(hRookLoc.x, hRookLoc.y)->getPiece();
                ...
                if (hRook != nullptr && !hRook->getHasMoved()) {
                    ...
                    if (canCastle) {
                        result.push_back(board->getTileUnsafe(hRookLoc.x, hRookLoc.y));
                    }
                }
            }
        }
    ...
    bool King::isChecked(Board* board) {
        TileLocation loc = board->getPiecePosition(this->getId());
        Tile* tile = board->getTileUnsafe(loc.x, loc.y);
    ...
    void Knight::getPseudoLegalMoves(std::vector<Tile*> &result, Board* board) {
        ...
        for (const DirectionValue &direction : knightDirections) {
            ...
            if (tile != nullptr) {
                ...
                if ((piece == nullptr ||
                     (tile->getPiece()->getPieceType() != PieceType::KING && piece->getColor() != this->getColor()))) {
                    result.push_back(tile);
                }
            }
        }
...
/*        for (Piece* defender : ownTile->getAttackersByColor(getColor())) {
    ...
    void Pawn::getPseudoLegalMoves(std::vector<Tile*> &result, Board* board) {
        ...
        Tile* frontTile = board->getTileUnsafe(loc.x, loc.y + direction);

        if (frontTile->getPiece() == nullptr) {
            result.push_back(frontTile);

            if (!this->getHasMoved()) {
                Tile* secondFrontTile = board->getTileUnsafe(loc.x, loc.y + direction * 2);

                if (secondFrontTile->getPiece() == nullptr) {
        ...
        for (Tile* attackedTile : attackedTiles) {
            if ((attackedTile->getEnPassantTarget() != nullptr &&
                 attackedTile->getEnPassantTarget()->getColor() != this->getColor())
                || (attackedTile->getPiece() != nullptr && attackedTile->getPiece()->getColor() != this->getColor() &&
                    attackedTile->getPiece()->getPieceType() != PieceType::KING)) {
    ...
    void Pawn::getAttackedTiles(std::vector<Tile*> &result, Board* board) {
        ...
        for (Direction direction : this->getColor() == PieceColor::WHITE ? WHITE_PAWN_ATTACK_DIRECTIONS
                                                                         : BLACK_PAWN_ATTACK_DIRECTIONS) {
