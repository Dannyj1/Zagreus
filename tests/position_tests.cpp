...
const std::vector<std::string> POSITIONS = {"8/8/1n3k2/8/3P3P/5K2/8/1N4Q1 w - -",
...
TEST_CASE("test_PieceBitBoards", "[types]") {
    ...
    for (const std::string& fen : POSITIONS) {
        ...
        while (picker.next(move)) {
            ...
            if (board.getSideToMove() == WHITE) {
                overlappingPieces = board.getPieceBoard<WHITE_PAWN>() & board.getPieceBoard<WHITE_KNIGHT>() &
                                    board.getPieceBoard<WHITE_BISHOP>() & board.getPieceBoard<WHITE_ROOK>() &
                                    board.getPieceBoard<WHITE_QUEEN>() & board.getPieceBoard<WHITE_KING>();
                combinedPieces = board.getPieceBoard<WHITE_PAWN>() | board.getPieceBoard<WHITE_KNIGHT>() |
                                 board.getPieceBoard<WHITE_BISHOP>() | board.getPieceBoard<WHITE_ROOK>() |
                                 board.getPieceBoard<WHITE_QUEEN>() | board.getPieceBoard<WHITE_KING>();
            } else {
                overlappingPieces = board.getPieceBoard<BLACK_PAWN>() & board.getPieceBoard<BLACK_KNIGHT>() &
                                    board.getPieceBoard<BLACK_BISHOP>() & board.getPieceBoard<BLACK_ROOK>() &
                                    board.getPieceBoard<BLACK_QUEEN>() & board.getPieceBoard<BLACK_KING>();
                combinedPieces = board.getPieceBoard<BLACK_PAWN>() | board.getPieceBoard<BLACK_KNIGHT>() |
                                 board.getPieceBoard<BLACK_BISHOP>() | board.getPieceBoard<BLACK_ROOK>() |
                                 board.getPieceBoard<BLACK_QUEEN>() | board.getPieceBoard<BLACK_KING>();
            }
