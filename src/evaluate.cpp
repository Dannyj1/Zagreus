  ...
  while (colorBoard) {
    ...
    if (isKing(pieceType)) {
      if (color == WHITE) {
        ...
        whiteEndgameScore +=
            popcnt(virtualMobilitySquares) * getEvalValue(ENDGAME_KING_VIRTUAL_MOBILITY_PENALTY);
      } else {
        // Pawn Shield
    ...
    if (isBishop(pieceType)) {
      ...
      if (color == WHITE) {
        forwardMobility =
            bitboard.getRayAttack(index, NORTH_WEST) | bitboard.getRayAttack(index, NORTH_EAST);
      } else {
        forwardMobility =
            bitboard.getRayAttack(index, SOUTH_WEST) | bitboard.getRayAttack(index, SOUTH_EAST);
      }
