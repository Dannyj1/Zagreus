...
std::vector<TunePosition> loadPositions(
  ...
  for (std::string &posLine : lines) {
    ...
    if (tunerBoard.getMovingColor() == WHITE) {
      qScore = searchManager.quiesce<WHITE>(tunerBoard, -999999999, 999999999, rootMove, rootMove,
                                            maxEndTime, engine, true);
    } else {
      qScore = searchManager.quiesce<BLACK>(tunerBoard, -999999999, 999999999, rootMove, rootMove,
                                            maxEndTime, engine, true);
    }
