...
void initializePst() {
    for (Piece piece = WHITE_PAWN; piece <= BLACK_KING; piece++) {
        for (Square square = A1; square <= H8; square++) {
            midgamePst[piece][square] = midgameTableMapping[getPieceType(piece)][square];
            endgamePst[piece][square] = endgameTableMapping[getPieceType(piece)][square];
            midgamePst[piece][square] = midgameTableMapping[getPieceType(piece)][square ^ 56];
            endgamePst[piece][square] = endgameTableMapping[getPieceType(piece)][square ^ 56];
        }
    }
}
